from dataweaver.scraper.modules.pdf_scraper import (
    RequestsPDFScraper,
    PDFLinkExtractor,
    RequestsHttpClient,
)
...
def test_anchor_extraction_exception_logged(html_with_links):
    ...
    with patch("bs4.BeautifulSoup.find_all", side_effect=Exception("Erro")):
        with patch(
            "dataweaver.scraper.modules.pdf_scraper.logger.warning"
        ) as mock_warning:
...
def test_extract_handles_exception_from_anchors():
    ...
    with patch.object(
        PDFLinkExtractor,
        "_extract_from_anchors",
        side_effect=Exception("Erro em anchors"),
...
def test_extract_handles_exception_from_paragraphs():
    ...
    with patch.object(
        ...
        side_effect=Exception("Erro em paragrafos"),
    ), patch("dataweaver.scraper.modules.pdf_scraper.logger.error") as mock_logger:
...
def test_extract_handles_generic_exception():
    ...
    with patch.object(
        PDFLinkExtractor,
        "_extract_from_anchors",
        side_effect=Exception("Erro artificial"),
    ), patch("dataweaver.scraper.modules.pdf_scraper.logger.error") as mock_logger:
...
def test_http_client_raises_on_failure():
    ...
    with patch(
        "requests.get", side_effect=requests.exceptions.RequestException("Erro")
    ):
        client = RequestsHttpClient()
        with patch(
            "dataweaver.scraper.modules.pdf_scraper.logger.error"
        ) as mock_logger:
